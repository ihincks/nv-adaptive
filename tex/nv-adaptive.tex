...
Let $U(\mps,\data,\eps)$ be the utility of collecting the datum
$\data$ under configuration $\eps$ given the hypothetical
model parameters $\mps$.
Then according to our current state of knowledge $\pi_n(\mps)$,
using the Bayesian maxim of marginalizing over unknown quantities,
the average utility of observing $\data$ under 
...
For our purposes, an experiment is fully specified as
\begin{align}
    \eps=(a(t), \omega_\uw, N)
\end{align}
where $a(t):[0,\te]\rightarrow [-1,1]$ is the 
unitless pulse profile yielding $\Omega(t)=a(t)\Omega_0$,
$\omega_\uw$ is the applied microwave frequency,
and $N$ is the number of repetitions of this experiment\footnote{The
experiment configuration must also specify value for each timing
found in \autoref{fig:experiment-pulse-sequences}, but as they are
calibrated independently from the experiment of interest, we omit
them here for simplicity.}.
We further restrict our attention to only two types of
simple experiments, depicted in \autoref{fig:experiment-pulse-sequences},
given by
\begin{enumerate}
...
an experiment configuration $\eps$, the super operator
...
This leads to the measurement probability
\begin{align}
    p(\mps,\eps)
