...
choice to depend on what has already been done.
...
were general.
In \autoref{sec:computation-and-hardware} we discuss 
...
This set of parameters includes both parameters of interest and nuisance parameters, which are
...
made using a fixed experiment configuration.
...
As an example, conventionally, Rabi curves are generated 
...
Based on this quantity we choose the next experiment to be
the one that maximizes utility,
\begin{align}
    \eps_{n+1} = \argmax_\eps U_n(\eps),
\end{align}
with the maximum taken over some space of possible experiments.
If computed numerically, we might expect to find a local maxima.
...
\end{align}
and $Q$ is a positive semi-definite weighting matrix.
...
enough compared to the parallelized simulation cost. For our setup and count rates, we landed naturally in this regime with
CPU computation performed on a single desktop computer.}.
This causes the side-effect where the next experiment being chosen with
information that is one cycle out-of-date; however,
for our simulations and data collection rates,
we found that this did not have a noticeable
effect on learning rates.
After the design of a new experiment configuration $\eps_{n+1}$ having been 
...
that mix into an IQ circuit. %(Marki IQ0255LMP).
...
Ramsey experiments.
Put differently, given that our free evolution commutes with 
...
almost entirely insensitive at low field with linearly
...
numerical stability of the SMC sampler;  alternation 
does not affect the overall information content as individual
...
once transient
behavior has settled down; the dotted line in \subref{fig:heuristic-comparison}{(d)} guides the eye with the 
...
We can now ask the question: how many times, $M$, must we measure 
...
with strength $m_I A$, where $m_I\in\{-1,0,+1\}$ is its current eigenstate. This is justified due to its long $T_1$ \todo{insert estimate and reference for time.} time with respect to a single
experiment.
...
\end{align}
\todo{Possibly explicitly describe outcome sampling protocol}
